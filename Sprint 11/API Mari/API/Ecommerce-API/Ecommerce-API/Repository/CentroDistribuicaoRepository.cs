...
public class CentroDistribuicaoRepository : ICentroDistribuicaoRepository
{
    ...
    public CentroDistribuicaoRepository(EcommerceContext context, IMapper mapper, IConfiguration configuration)
    {
        _context = context;
        _mapper = mapper;
    ...
    private CentroDistribuicao BuscarPorId(int id)
    {
        return _context.CentrosDistribuicoes.FirstOrDefault(centro => centro.Id == id);
    }

   public async Task<CentroDistribuicao> CadastrarCentroDistribuicao(CreateCentroDistribuicaoDto centroDto)
    {
        CentroDistribuicao centro = _mapper.Map<CentroDistribuicao>(centroDto);
