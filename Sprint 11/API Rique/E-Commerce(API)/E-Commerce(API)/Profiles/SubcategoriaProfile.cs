...
public class SubcategoriaProfile : Profile
{
    public SubcategoriaProfile()
    {
        ...
        CreateMap<Subcategoria, ReadSubcategoriaDto>()
            ...
            .MapFrom(subcategoria => subcategoria.DataEHoraCriacao.ToString("dd/MM/yyyy HH:mm:ss")))
            .ForMember(subcategoriaDto => subcategoriaDto.Modificacao, opt => opt.MapFrom(subcategoria => subcategoria.DataEHoraModificacao == new DateTime() ? "Não houve modificação" : subcategoria.DataEHoraModificacao.ToString("dd/MM/yyyy HH:mm:ss")));
