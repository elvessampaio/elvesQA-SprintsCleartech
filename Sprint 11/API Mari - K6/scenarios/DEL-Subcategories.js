...
export function testDelSubcategoria(){
  let ids = idList;

  // Analisa o array de IDs e realiza a requisição DELETE para cada
  for (let i = 0; i < ids.length; i++) {
    let response = http.del(`https://localhost:7161/Subcategoria/${ids[i]}`);

    check(response, {
      [`Status é 204 - DELETE Subcategoria ID ${ids[i]}`]: (r) => r.status === 204,
      [`Duração máxima - DELETE Subcategoria ID ${ids[i]}`]: (r) => r.timings.duration < 1000,
    });

    let responseTime = response.timings.duration;

    let metricsDelSubcategoria = {
