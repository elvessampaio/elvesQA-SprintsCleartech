...
public class CarrinhoDeComprasController : ControllerBase
{
    ...
    public async Task<IActionResult> AddEnderecoDeEntregaAoCarrinho(Guid carrinhoDeComprasId, AddEnderecoDeEntregaDto enderecoDto)
    {
    ...
    [HttpPut("{carrinhoDeComprasId}/enderecoDeEntrega")]
    public IActionResult EditarEnderecoDeEntregaDoCarrinho(Guid carrinhoDeComprasId, [FromBody] UpdateEnderecoDeEntregaDto updateDto)
    {
        var carrinho = _service.EditarEnderecoDeEntregaDoCarrinho(carrinhoDeComprasId, updateDto);

        return Ok(carrinho);
    }

    [HttpPut("{carrinhoDeComprasId}/produto/{produtoId}")]
    public IActionResult AlterarQuantidadeDeProdutosNoCarrinho(Guid carrinhoDeComprasId, Guid produtoId,
